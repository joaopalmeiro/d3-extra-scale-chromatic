export { default as schemeCarbonDark14 } from "./categorical/CarbonDark14";
export { default as schemeCarbonLight14 } from "./categorical/CarbonLight14";
export { default as schemeCategory20 } from "./categorical/Category20";
export { default as schemeCategory20b } from "./categorical/Category20b";
export { default as schemeCategory20c } from "./categorical/Category20c";
export { default as schemeFlatUI } from "./categorical/FlatUI";
export { default as schemePajamasDark } from "./categorical/PajamasDark";
export { default as schemePajamasLight } from "./categorical/PajamasLight";
export { default as schemePlotly } from "./categorical/Plotly";
export { default as schemeTableau20 } from "./categorical/Tableau20";
